...
def main(arglist):
    ...
    for j in range(VALIDATION_SET_SIZE):
        ...
        for k in range(VALIDATION_SET_LOOKAHEAD):
            ...
            next_state, _, error = control_env.perform_action(temp_state, temp_action)
            if error is None:
    ...
    if plan_type == 'vi':
        ...
        vs_policy = {vs: GameEnv.WALK_LEFT for vs in val_states}
    ...
    for trial in range(EPISODE_TRIALS):
        ...
        visualising_trial = visualise and trial == 0

        if visualising_trial:
            try:
        ...
        while not control_env.is_solved(persistent_state) and \
            not control_env.is_game_over(persistent_state) and \
            (visualising_trial or episode_reward > control_env.reward_min_tgt) and \
            (not visualising_trial or episode_steps < MAX_ITERATIONS):
            ...
            random.seed(seed)
            persistent_state, reward, err_msg = control_env.perform_action(persistent_state, action)
            is_terminal = control_env.is_game_over(persistent_state)
